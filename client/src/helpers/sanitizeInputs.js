...
export function listenToActiveOnly(code, state) {
  if ( (code === 'KeyW' && state.KeyW) || (code === 'KeyA' && state.KeyA) 
    || (code === 'KeyS' && state.KeyS) || (code === 'KeyD' && state.KeyD)) 
    return true;

  if ( (code === 'ArrowUp' && state.ArrowUp) || (code === 'ArrowLeft' && state.ArrowLeft)
    || (code === 'ArrowDown' && state.ArrowDown) || (code === 'ArrowRight' && state.ArrowRight))
    return true;
